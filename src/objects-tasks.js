...
function mergeObjects(objects) {
  ...
  objects.forEach((obj) => {
    Object.entries(obj).forEach(([key, value]) => {
      newObject[key] = (newObject[key] || 0) + value;
    });
  });
...
function makeWord(lettersObject) {
  const arrProp = Object.entries(lettersObject);
  const arrResult = arrProp.reduce((acc, [letter, positions]) => {
    positions.forEach((i) => {
      acc[i] = letter;
    });
    return acc;
...
function sellTickets(queue) {
  const result = queue.reduce(
    (acc, cur) => {
      ...
      if (typeof acc[cur] === 'number') acc[cur] += 1; // если пришла 25 или 50 добавить в кассу

      let money = cur;
      while (money > 25 && (acc[50] || acc[25])) {
        if (money > 50 && acc[50] > 0) {
          money -= 50;
          acc[50] -= 1;
        }
        if (money > 25 && acc[25] > 0) {
          money -= 25;
          acc[25] -= 1;
        }
      }

      if (money === 25) return acc;
      return null;
    },
    { 50: 0, 25: 0 }
  );
